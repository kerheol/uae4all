...
static _INLINE_ void init_aspect_maps (void)
{
    ...
    native2amiga_line_map = (int *)xmalloc (sizeof (int) * gfxHeight);
	...
	if (v >= gfxHeight && max_drawn_amiga_line == -1)
	    max_drawn_amiga_line = i - min_ypos_for_screen;
	if (i < min_ypos_for_screen || v >= gfxHeight)
	    v = -1;
...
*/
	...
	if (amiga2aspect_line_map[i] == -1)
	    continue;
	for (j = amiga2aspect_line_map[i]; j < gfxHeight && native2amiga_line_map[j] == -1; j++)
...
static _INLINE_ void finish_drawing_frame (void)
{
	...
	if (where >= mainMenu_displayedLines - ((mainMenu_showStatus) ? TD_TOTAL_HEIGHT : 0))
#else
	if (where >= gfxHeight - TD_TOTAL_HEIGHT)
#endif
	    break;
	if (where == -1)
	    continue;
