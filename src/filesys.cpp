...
#	define MAX_DIR_LEVELS 128
	...
	a_inode *node_root[MAX_DIR_LEVELS];
	a_inode *node;
	int level = 0;
	
	/* Populate directory nodes. If directory has never been opened, directory tree
	 * will be empty, and we won't be able to traverse it
	 */
	populate_directory (unit, root);
	
	node_root[level++] = root;
	node = root->child;
	
	/* All the files and directories in the root must be deleted before
	 * root can be deleted
	 */
	while (level > 0) {
		while ((node != NULL) && (node->dir == 1)) {
			if (level < MAX_DIR_LEVELS - 1) {
				/* Populate directory nodes */
				populate_directory (unit, node);
				
				/* Remember the root and move to the lower level */
				node_root[level++] = node;
				node = node->child;
			} else
				return -1;
		}
		
		/* Delete files on this level until another directory is found */
		while ((node != NULL) && (node->dir == 0)) {
			if (unlink (node->nname) == -1)
				return -1;
			node = node->sibling;
		}
		
		if (node == NULL) {
			/* No more siblings => directory is empty and can now be deleted */
			node = node_root[--level];
			if (rmdir (node->nname) == -1)
				return -1;
			node = node->sibling;
		}
	}
...
static uae_u32 filesys_diagentry (void)
{
    ...
    residents = resaddr;    while (tmp < residents && tmp > start) {
