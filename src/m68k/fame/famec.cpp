...
#if defined(GCW0) && !defined(NDEBUG)
#define NDEBUG
#endif
...
#define M68K_ADR_MASK  ((1 << FAME_ADDR_BITS)-1)
#else
...
#if !defined(FAME_NO_RESTORE_PC_MASKED_BITS) && FAME_ADDR_BITS < 32
#define RESTORE_PC_MASKED_BITS(_ADR_) (((unsigned)(_ADR_))| ((((unsigned)(_ADR_)) >> (FAME_ADDR_BITS-1))?0xFFFFFFFF^M68K_ADR_MASK:0))
#else
#define RESTORE_PC_MASKED_BITS(_ADR_) (_ADR_)
#endif
...
#define SET_FLAGS_DIV_OVERFLOW \
	flag_C = 0; \
	flag_V = M68K_SR_V; \
	/* these flags are undefined when there is division overflows */ \
	/* but are set here to match FAME versions */ \
	flag_NotZ = 1; \
	flag_N = 0;
...
static EXTRA_INLINE void execute_exception(s32 vect)
{
    ...
    if (FAME_CONTEXT.icust_handler && FAME_CONTEXT.icust_handler[vect])
    {
        ...
        FAME_CONTEXT.pc = RESTORE_PC_MASKED_BITS(GET_PC);
    ...
    else
    {
        ...
        oldPC = (u32)(PC) - BasePC;
        PUSH_32_F(RESTORE_PC_MASKED_BITS(oldPC))
        PUSH_16_F(oldSR)
...
{
    return (FAME_CONTEXT.execinfo & M68K_RUNNING)?RESTORE_PC_MASKED_BITS((u32)PC-BasePC):FAME_CONTEXT.pc;
}
...
s32 FAME_API(fetch)(u32 addr, u32 memory_space)
{
    ...
    else
    {
        ...
        u16 *p= (u16*)(((addr) & M68K_ADR_MASK) + (tmp));
        val = *p;
#ifdef FAME_DEBUG
        printf("@%08X *%p=%04X\n",addr,p,val);
#endif
