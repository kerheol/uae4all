...
extern int hwScaled;
//extern int resChange;
#include "gp2x.h"
#include "gp2xutil.h"
#endif

#ifdef PANDORA
extern FILE* fPndButtonMode;
...
#if defined (GP2X) || defined (GIZMONDO) || defined (PSP) || defined(GCW0)
#include "inputmode.h"
#ifdef GP2X
#if !defined(PANDORA) && !defined(GCW0)
...
#ifdef PANDORA
SDL_Surface *prSDLScaleScreen = NULL;
SDL_Surface *backbuffer = NULL;
SDL_Rect srcrect200h = {0,0,320,200};
SDL_Rect srcrect216h = {0,0,320,216};
SDL_Rect srcrect256h = {0,0,320,256};
SDL_Rect dstrect768w = {0,0,768,480};
SDL_Rect dstrect716w = {26,0,716,480};
SDL_Rect dstrect640w = {64,0,640,480};
SDL_Rect dstrect608w = {80,0,608,480};
//screenmode 0:320x256 softstretch, 1:320x216 softstretch
//2:320x200 softstretch, 3: NTSC (320x200 softstretched to 4:3 format)
#ifdef SOFTSTRETCH
int screenmode = 0;
#else
int screenmode = -1;
...
void flush_block (int ystart, int ystop)
{
	...
	updatescreen( prSDLScreen, 0, ystart, current_width, ystop-ystart+1 );
#endif
    if (drawfinished)
    {
	drawfinished=0;
...
static void graphics_subinit (void)
{
	...
	uiSDLVidModFlags = SDL_SWSURFACE; //SDL_SWSURFACE;
#ifdef DEBUG_GFX
...
#else
	{
		...
		current_width = 768;
		current_height = 480;
		prSDLScreen = SDL_SetVideoMode(768, 480, 16, uiSDLVidModFlags|VIDEO_FLAGS);
		prSDLScaleScreen = SDL_SetVideoMode(768, 480, 16, uiSDLVidModFlags|VIDEO_FLAGS);
	#else
		//prSDLScreen = SDL_SetVideoMode(320, 200, 16, uiSDLVidModFlags|VIDEO_FLAGS);
		//prSDLScreen = SDL_CreateRGBSurface(SDL_SWSURFACE,current_width,current_height,16,0,0,0,0);
		//prSDLScaleScreen = SDL_SetVideoMode(current_width*2, current_height*2, 16, uiSDLVidModFlags|VIDEO_FLAGS);
	#endif
#else
		prSDLScreen = SDL_SetVideoMode(current_width, current_height, 16, uiSDLVidModFlags|VIDEO_FLAGS);
		printf("prSDLScreen is null, creating new screen\n");
#endif
	}
#endif
	...
	{
		current_height=205;
		prSDLScreen = SDL_SetVideoMode(320, 205, 16, uiSDLVidModFlags|VIDEO_FLAGS);
		resChange = 0;
	}*/
...
show_volumecontrol = 0;
	...
	if(enableHWscaling)
	{
		current_width = PREFS_GFX_SCALED_WIDTH;//PREFS_GFX_WIDTH;
		current_height = PREFS_GFX_SCALED_HEIGHT;//PREFS_GFX_HEIGHT;
	}
	else
	{
#endif
		current_width = PREFS_GFX_WIDTH;//PREFS_GFX_WIDTH;
		current_height = PREFS_GFX_HEIGHT;//PREFS_GFX_HEIGHT;
#ifndef PANDORA
	}
#endif
//	PREFS_GFX_SCALED_WIDTH,PREFS_GFX_SCALED_HEIGHT
	graphics_subinit ();
...
printf("%i\n",cuenta); fflush(stdout);
...
#if defined(PANDORA) || defined(GCW0)
	if(rEvent.jbutton.button==GP2X_BUTTON_Y)
		slow_mouse = true;
	...
	if(gp2xMouseEmuOn)
	{
		...
		if(rEvent.jbutton.button==GP2X_BUTTON_X)
		{
			uae4all_keystate[AK_DN] = 1;
			record_key(AK_DN << 1);
		}
		if(rEvent.jbutton.button==GP2X_BUTTON_A)
		{
		...
		if(rEvent.jbutton.button==GP2X_BUTTON_B)
		{
		...
		{
			if (rEvent.jbutton.button==GP2X_BUTTON_L || rEvent.jbutton.button==GP2X_BUTTON_R)
	    ...
	    dbg("Event: key down");
#endif
#if !defined (DREAMCAST) && !defined (GP2X) && !defined(GCW0)
...
/*#if defined(GP2X)
	    mouse_state = true;
	    mouse_x = rEvent.motion.x/2; //(rEvent.motion.x/prSDLScaleScreen->w) * prSDLScreen->w;
	    mouse_y = rEvent.motion.y/2; //(rEvent.motion.y/prSDScaleScreen->h) * prSDLScreen->h;
	    newmousecounters = 1;
#endif*/
...
/*#if defined(GP2X)
	...
	case SDL_MOUSEMOTION:
		...
		/*lmx = lastmx;
		lmy = lastmy;*/
		lastmx = rEvent.motion.x - mainMenu_stylusOffset + moved_x + stylusAdjustX >> 1;
		lastmy = rEvent.motion.y - mainMenu_stylusOffset + moved_y + stylusAdjustY >> 1;
		/*lmx = lastmx-lmx;
		lmy = lastmy-lmy;
		if(lmx < -100 || lmx > 100 || lmy < -100 || lmy > 100)
		    framesUntilClick = 17;
		else if(lmx > -20 && lmx < 20 && lmy > -20 && lmy < 20)
		    framesUntilClick = 5;
		else
		    framesUntilClick = 10;*/
		mouseMoving = 1;
	    ...
	    {
		lastmx += rEvent.motion.xrel / 4;
		lastmy += rEvent.motion.yrel / 4;
	    }
	    else
	    {
		if(mainMenu_mouseMultiplier < 5)
		{
			lastmx += rEvent.motion.xrel * mainMenu_mouseMultiplier;
			lastmy += rEvent.motion.yrel * mainMenu_mouseMultiplier;
		}
		else if(mainMenu_mouseMultiplier == 50)
		{
			lastmx += rEvent.motion.xrel / 2;
			lastmy += rEvent.motion.yrel / 2;
		}
		else
		{
			lastmx += rEvent.motion.xrel / 4;
...
void updatescreen( SDL_Surface* screen, int x, int y, int width, int height )
{
	if(hwScaled)
	{
		//wait for vsync
...
if (screenmode < 0)
{
	if (screen != NULL && prSDLScaleScreen != NULL)
	{
		int depth = prSDLScaleScreen->format->BytesPerPixel;

		if (enableHWscaling)
		{
		      scale(2, prSDLScaleScreen->pixels, screen->w*depth*2, screen->pixels, screen->w*depth,
			    depth, screen->w, screen->h );
		}
		else
		{
		      Simple2x((unsigned char*)screen->pixels, screen->w*depth, 0, (unsigned char*)prSDLScaleScreen->pixels, screen->w*depth*2,
			       screen->w, screen->h, mainMenu_scanlines);
		}
	//wait for vsync
	arg = 0;
	ioctl(fbdev, FBIO_WAITFORVSYNC, &arg);
	...
	#else
		SDL_UpdateRect(prSDLScaleScreen, 0, 0, 0, 0);
	#endif
	}
	else
	{
...
else
{
	...
	backbuffer = SDL_DisplayFormat(prSDLScreen);
	SDL_FillRect(prSDLScaleScreen,NULL,0);
	//0:320x256 softstretch, 1:320x216 softstretch
	//2:320x200 softstretch, 3: NTSC (320x200 softstretched to 4:3 format)
	switch(screenmode)
	{
        case 0: SDL_SoftStretch(backbuffer,&srcrect256h,prSDLScaleScreen,&dstrect608w); break;
		case 1: SDL_SoftStretch(backbuffer,&srcrect216h,prSDLScaleScreen,&dstrect716w); break;
		case 2: SDL_SoftStretch(backbuffer,&srcrect200h,prSDLScaleScreen,&dstrect768w); break;
		case 3: SDL_SoftStretch(backbuffer,&srcrect200h,prSDLScaleScreen,&dstrect640w);
	}
	//wait for vsync
	arg = 0;
	ioctl(fbdev, FBIO_WAITFORVSYNC, &arg);
