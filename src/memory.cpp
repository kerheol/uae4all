...
char *kickstarts_rom_names[] = { "kick12.rom\0", "kick13.rom\0", "kick20.rom\0" };
...
{
    int i, custom_start;
	if (mainMenu_chipMemory == 2) prefs_chipmem_size=0x00200000;
	else if (mainMenu_chipMemory) prefs_chipmem_size=0x00100000;
	else prefs_chipmem_size=0x00100000>>1;

	if (mainMenu_bogoMemory == 2) prefs_bogomem_size=0x00100000;
	else if (mainMenu_bogoMemory) prefs_bogomem_size=0x00100000>>1;
	else prefs_bogomem_size=0x00000000;
#ifdef DEBUG_MEMORY
    dbg("memory_reset!");
#endif
#ifdef NATMEM_OFFSET
